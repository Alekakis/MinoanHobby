...
export default async function handler(req, res) {
	...
	try {
		...
		if (req.method === 'GET') {
			...
			if (!exists) await redis.set(REDIS_KEY, MAX_STOCK); // initialize
		...
		if (req.method === 'POST') {
			...
			const exists = await redis.exists(REDIS_KEY);
			if (!exists) await redis.set(REDIS_KEY, MAX_STOCK);
			...
			if (action === 'remove') {
				...
				if (newStock > MAX_STOCK) {
					await redis.set(REDIS_KEY, MAX_STOCK);
					return res.status(200).json({ success: true, stock: MAX_STOCK });
				}
				return res.status(200).json({ success: true, stock: newStock });
			}
